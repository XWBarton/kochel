...
def resolve_upload_destination(relative_path: str) -> Path:
    ...
    destination under MUSIC_LIBRARY_ROOT.

    Raises UploadRejected for anything that looks like a path-traversal
    attempt, an absolute path, or an unsupported file type — this path is
    driven entirely by client-supplied filenames, so it's treated as
    untrusted input regardless of the single-user trust model elsewhere.
    """
    ...
    if candidate.suffix.lower() not in AUDIO_EXTENSIONS:
        raise UploadRejected(f"unsupported file type: {candidate.suffix or '(none)'}")
